 ...
 * @version 1.0
 */
public class SaasPreviewThemeAction {
	...
	public static String getPreviewTheme(HttpServletRequest request) {
		return (String) request.getSession().getAttribute(SaasPreviewThemeAction.previewTheme);
	}

	public static boolean hasPreviewTheme(HttpServletRequest request) {
		HttpSession session = request.getSession();
		return session.getAttribute(SaasPreviewThemeAction.previewTheme) != null;
	}
	...
	public static void doSetPreviewTheme(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (themeName != null && !"".equals(themeName)) {
			request.getSession().setAttribute(SaasPreviewThemeAction.previewTheme, themeName);
		}
