package com.intkr.saas.util.img;
...
public class ImageUtils {
	...
	public void makeImage(BufferedImage newImage) throws IOException {
		...
		if (fileExtension.equalsIgnoreCase("jpg") || fileExtension.equalsIgnoreCase("jpeg") || fileExtension.equalsIgnoreCase("bmp")) {
	...
	public static void main(String[] args) {
		...
		for (File theme : themes) {
			...
			process(srcImage, descImage);
		}
	}

	private static void process(String srcImage, String descImage) {
		String str = srcImage;
		File f = new File(str); // 原图片

		ImageUtils.fromFile(f) // 设置原图片
				.width(400) // 设置生成图片的宽度，高度将以原图片的高度等比例伸缩
				// .height(200) //设置生成图片的高度，宽度将以原图片的宽度等比例伸缩
				// .scale(1) // 设置生成图片的伸缩比例
				// .size(200, 22) // 设置生成图片的宽度和高度
				// .rotate(34) // 设置原图片的旋转角度
				// .bgcolor(Color.BLUE) // 设置背景颜色，设置成null表示不处理背景颜色
				.quality(1f) // 设置压缩比例
				.toFile(new File(descImage)); // 生成图片的路径
	}
}