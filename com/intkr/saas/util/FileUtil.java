...
public class FileUtil {
	...
	public static interface ChangeProcessor {
		public String process(String line);
	}

	public static void change(String fileName, ChangeProcessor process) {
		String content = "";
		...
		try {
			...
			while ((tempString = reader.readLine()) != null) {
				content += process.process(tempString) + "\n";
			}
		} catch (Exception e) {
	...
	private static void write(String fileName, String content, boolean append) {
		...
		File directory = new File(fileTmp.getAbsolutePath().substring(0, fileTmp.getAbsolutePath().lastIndexOf(File.separator)));
		if (!directory.exists()) {
			directory.mkdirs();
		}
	...
	public static void write(String fileName, byte[] content) {
		...
		try {
			...
			if (file.exists()) {
				file.delete();
				bw = new FileOutputStream(file, true);
			} else {
	...
	public static File findDirectory(File desc, String directory, Integer level) {
		level++;
		if (level > 7) {
			return null;
		}
		if (desc == null) {
			...
			if (desc.getName().equals(directory)) {
				return desc;
			}
			File[] files = desc.listFiles();
			for (File child : files) {
				File file = find(child, directory, level);
				if (file != null) {
					return file;
				}
			}
	...
	public static File findSubDirectory(File desc, String directory) {
		if (desc == null) {
			return null;
		} else if (desc.isDirectory()) {
			File[] files = desc.listFiles();
	...
	public static void main(String[] args) {
		File file = find(new File("D:/workspace/IK/src/main/webapp"), "IntKrSecret", 1);
