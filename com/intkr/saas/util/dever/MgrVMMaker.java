...
public class MgrVMMaker {
	...
	public static void maker() {
		...
		String content = TemplateUtil.merge(TemplateEngine.getContent("template/MgrVMTemplate.vm"), param);
	...
	private static String makeTableHeader(TableDO table) {
		...
		for (ColumnDO column : table.getColumns()) {
			...
			String template = "\n\t\t\t<th order-field=\"{{name}}Column\">{{desc}}</th>";
			template = template.replace("{{desc}}", column.getDesc());
			template = template.replace("{{name}}", column.getProName());
			template = template.replace("{{dtoName}}", column.getProName());
			string += template;
